...
(function () {
	...
	var FILENAME_PROP = "{http:// /ns}trashbin-filename";
	var DELETION_TIME_PROP = "{http:// /ns}trashbin-deletion-time";
	var TRASHBIN_ORIGINAL_LOCATION = "{http:// /ns}trashbin-original-location";
	var TRASHBIN_TITLE = "{http:// /ns}trashbin-title";
	...
	FileList.prototype = _.extend(
		...
		/** @lends OCA.Trashbin.FileList.prototype */ {
			...
			initialize: function () {
				...
				var result = OCA.Files.FileList.prototype.initialize.apply(
					this,
					arguments
				);
				this.$el
					.find(".undelete")
					.click("click", _.bind(this._onClickRestoreSelected, this));
				...
				this.breadcrumb._makeCrumbs = function () {
					var parts =
						OCA.Files.BreadCrumb.prototype._makeCrumbs.apply(this, [
							...arguments,
							"icon-delete no-hover",
						]);
			...
			_setCurrentDir: function (targetDir) {
				OCA.Files.FileList.prototype._setCurrentDir.apply(
					this,
					arguments
				);
			...
			getAjaxUrl: function (action, params) {
				...
				return (
					OC.filePath("files_trashbin", "ajax", action + ".php") + q
				);
			},
			...
			linkTo: function (dir) {
				return (
					OC.linkTo("files", "index.php") +
					"?view=trashbin&dir=" +
					encodeURIComponent(dir).replace(/%2F/g, "/")
				);
			},
			...
			_onClickRestoreSelected: function (event) {
				...
				return Promise.all(restorePromises).then(
					function () {
						self.fileMultiSelectMenu.toggleLoading(
							"restore",
							false
						);
					},
					function () {
						OC.Notification.show(
							t(
								"files_trashbin",
								"Error while restoring files from trash bin"
							)
						);
					}
				);
			...
			_onClickDeleteSelected: function (event) {
				...
				if (allFiles) {
					return this.client
						.remove(
							OC.joinPaths("trash", this.getCurrentDirectory())
						)
						.then(
							function () {
								self.hideMask();
							...
							function () {
								OC.Notification.show(
									t(
										"files_trashbin",
										"Error while emptying trash bin"
									)
								);
							}
						);
			...
			_onClickFile: function (event) {
				...
				return OCA.Files.FileList.prototype._onClickFile.apply(
					this,
					arguments
				);
			},
			...
			generatePreviewUrl: function (urlSpec) {
				return (
					OC.generateUrl("/apps/files_trashbin/preview?") +
					$.param(urlSpec)
				);
			},
			...
			_getWebdavProperties: function () {
				return [
					FILENAME_PROP,
					DELETION_TIME_PROP,
					TRASHBIN_ORIGINAL_LOCATION,
					TRASHBIN_TITLE,
				].concat(this.filesClient.getPropfindProperties());
			},
			...
			reloadCallback: function (status, result) {
				...
				if (status === 403) {
					...
					OC.Notification.show(
						t("files", "This operation is forbidden")
					);
					return false;
				}
